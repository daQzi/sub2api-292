...
func (s *adminServiceImpl) UpdateAccount(ctx context.Context, id int64, input *UpdateAccountInput) (*Account, error) {
	...
	if input.Extra != nil {
		...
		// Re-auth and older clients may replace extra without knowing this setting.
		// An omitted flag must not silently re-enable a previously excluded account.
		if _, provided := input.Extra[openAICodexTicketEnabledExtraKey]; !provided {
			if enabled, exists := account.Extra[openAICodexTicketEnabledExtraKey].(bool); exists {
				if normalizedExtra == nil {
					normalizedExtra = make(map[string]any)
				}
				normalizedExtra[openAICodexTicketEnabledExtraKey] = enabled
			}
		}
	}
